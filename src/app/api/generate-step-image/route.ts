...
function generateStepPrompt(
  ...
  // アクリル絵具の線画質感
  const lineArtStyle = "不透明絵の具に適した大胆で自信のある線";

  // アクリル絵具の色塗り質感
  const coloringTexture =
    "不透明な塗り、わずかな筆の質感、豊かな色の彩度、マットから半光沢の仕上がり";

  // アクリル絵具のハイライト質感
  const highlightTexture =
    "不透明な白いハイライト、わずかなインパスト質感、鮮明なエッジ";

  // アクリル絵具の影質感
  const shadowTexture = "筆ストロークの影、不透明な暗部、質感のある塗り";
...
元画像の正確な形とレイアウトを保持し、アクリル絵具に適した線品質を持つ精密な線画を生成してください。`;
  }
...
入力: 元画像 + 線画画像
目的: 元画像から抽出した色で線画の内側のみを塗り、アクリル絵具の質感を適用
...
画材特有の色塗り技法:
- アクリル絵具の色塗り特性を適用: ${coloringTexture}
- アクリルメディウムに適した色の適用スタイル
- アクリル絵具に適した色の彩度とカバレッジを維持
...
画材特有のハイライト技法:
- アクリル絵具のハイライト特性を適用: ${highlightTexture}
