...
import { StepGenerationResponse, ImageAnalysisResponse } from '@/types/analysis';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // タイムアウト付きで手順生成実行
    const generationPromise = geminiService.generateSteps(imageUrl, material, analysisResult as ImageAnalysisResponse);
    ...
    try {
      const stepsResult = await Promise.race([generationPromise, timeoutPromise]);

      // 手順生成結果の検証
      if (!stepsResult.steps || !Array.isArray(stepsResult.steps) || stepsResult.steps.length === 0) {
      ...
      const totalTime = validatedSteps.reduce((sum, step) => sum + step.estimatedDuration, 0);
      ...
      if (error instanceof Error && error.message === 'TIMEOUT') {
        const errorResponse: ApiResponse<null> = {
          ...
          error: {
            code: 'TIMEOUT_ERROR',
            message: '手順生成に時間がかかりすぎています。しばらく待ってから再試行してください。',
          },
        };
    ...
    let errorMessage = '手順の生成に失敗しました。しばらく待ってから再試行してください。';
    let errorCode = 'GENERATION_ERROR';

    if (error instanceof Error) {
      if (error.message.includes('API key') || error.message.includes('quota')) {
        errorMessage = 'AI手順生成サービスでエラーが発生しました。しばらく待ってから再試行してください。';
