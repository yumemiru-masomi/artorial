...
function generateLandscapeBackgroundPrompt(stepDescription: string): string {
  return `**【風景画背景専用塗り】複雑な背景も正確に再現してください**

【実行する手順】
"${stepDescription}"

**【重要なルール】**
1. **前景の線画・輪郭線** → 黒い線のまま完全に保持する（消さない・薄くしない）
2. **前景の物体** → 白いまま残す（塗らない）
3. **背景の部分のみ** → 元画像と同じ色で塗る
4. **線画の保持** → すべての黒い線（建物・木・人物の輪郭）は鮮明に残す

**【風景背景の詳細再現】**
- **空の表現**：グラデーション、雲の形、時間帯による色変化を再現
- **遠景の山・建物**：距離感による色の変化、霞み、ボケ効果を表現
- **複雑な背景**：複数の要素（空、山、建物、植物）が混在する場合も正確に配置
- **光と影**：光源による明暗、反射、影の表現も考慮

**【技術的指示】**
1. **層別分析**：空、遠景、中景を分けて観察
2. **色の変化を追跡**：グラデーション、距離による色の変化
3. **境界の処理**：前景との境界を正確に認識
4. **奥行きの表現**：遠近感、大気遠近法の効果も再現

**重要：風景背景の複雑さを恐れず、元画像の美しさと奥行きを可能な限り再現してください！**`;
}
